...
def diffirent_color():
    return color_sensor.color(port.B) not in (-1, color_value)
...
# NL note: use lambda here because the function need access to color_value, which is not avaiable until main runs. 
    while True:
        ...
        if color_value == color.BLUE:
            ...
            await motor.run_for_degrees(port.E, 360, 400)
            ...
            light_matrix.show_image(light_matrix.IMAGE_HAPPY)
            motor.run_for_degrees(port.E, 360, 200)
            ...
            light_matrix.show_image(light_matrix.IMAGE_HAPPY)
            motor.run_for_degrees(port.E, 360, 600)
